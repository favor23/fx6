...
public class EchoHandler extends TextWebSocketHandler{
	...
	//방법 2 : 전체 채팅
	private List<List<WebSocketSession>> sessionList=new ArrayList<List<WebSocketSession>>();
	...
	public void afterConnectionEstablished(WebSocketSession session)
		...
		for(int i=0;i<sessionList.size();i++){
			...
			for(int j=0;j<str.length;j++){
				...
				logger.info("{} 연결됨"+i, session.getId()); 
				user.set(i, user.get(i)+"/"+session.getId());				
				}
			}	
		}
	}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		for (int i = 0; i < sessionList.size(); i++) {
			String str[] = user.get(i).split("/");
			for (int j = 0; j < str.length; j++) {
				if (session.getId().equals(str[j])) {
					...
					for (WebSocketSession sess : sessionList.get(i)) {
		...
		// 맵 방법.
		/*
		 * Iterator<String> sessionIds = sessions.ketSet().iterator(); String
		 * sessionId = ""; while (sessionIds.hasNext()) { sessionId =
		 * sessionIds.next(); sessions.get(sessionId).sendMessage(new
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		for (int i = 0; i < sessionList.size(); i++) {
			...
			for (int j = 0; j < str.length; j++) {
				if (session.getId().equals(str[j])) {
					// List 삭제
					//sessionList.get(i).remove(session);
