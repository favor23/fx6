...
<div class="cont_ba_opcitiy">
  ...
  <button class="btn_login" onclick="cambiar_login(1)">LOGIN</button>
  </div>
  </div>
...
<a href="#" onclick="ocultar_login_sign_up()" ><i class="material-icons">X</i></a>
   <h2>LOGIN</h2>
 <input type="text" placeholder="ID" id="input_id" />
<input type="password" placeholder="Password" id="input_pw"/>
...
<input type="hidden" name="id" id="hID" autocomplete="off">
<input type="hidden" name="pw" id="hPW" autocomplete="off">
</form>
...
function cambiar_login(data) {
	if(data==2){
		...
		$("#hID").attr("value",id);
		$("#hPW").attr("value",pw);
		$("#frm").attr("action","${pageContext.request.contextPath}/member/customerJoin");
		$("#frm").submit();
	}else {
